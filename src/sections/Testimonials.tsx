    ...
    <div className="relative z-10">
      ...
      <div className="flex mb-4">
        {[...Array(testimonial.rating)].map((_, i) => (
          <Star key={i} className="w-5 h-5 fill-current" style={{ color: '#AD2831' }} />
        ))}
      </div>
      
      <p className="text-gray-300 mb-6 italic leading-relaxed">"{testimonial.text}"</p>
      
      <div className="px-4 py-2 bg-gray-800/50 border rounded-lg inline-block"
           style={{ borderColor: '#AD2831' }}>
        <span className="font-bold text-sm" style={{ color: '#AD2831' }}>{testimonial.metric}</span>
      </div>
    </div>
  </div>
);
